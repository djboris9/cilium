...
func (kub *Kubectl) WaitforPods(namespace string, filter string, timeout time.Duration) error {
	...
	if err != nil {
		...
		ginkgoext.By("kubectl describe pods -n %s %s:\n=== STDOUT ===\n%s\n=== STDERR ===\n%s\n===\n",
			namespace, filter, desc.GetStdOut(), desc.GetStdErr())
	}
	return err
}
...
func (kub *Kubectl) DeleteCiliumDS() error {
	...
	// incomplete teardown.
	ns := GetCiliumNamespace(GetCurrentIntegration())
	ginkgoext.By("DeleteCiliumDS(namespace=%q)", ns)
	_ = kub.DeleteResource("ds", fmt.Sprintf("-n %s cilium", ns))
